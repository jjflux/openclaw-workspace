...
**2026-09-04 — Terms page rewritten to match the appointments-only offer** (commit 658e07f): `terms.html` Sections 1, 2, 3, 5, 6, 7, 8 replaced. Payment Terms now say pay per appointment, no subscription/setup/retainer/minimum/term, billable = HELD appointment unless a written agreement says otherwise, out-of-area/duplicate rules, card on file, 48-hour dispute window with credit or refund; Termination = either party any time by email, no fee, no notice period. SMS/TCPA sections 4, 9, 10 and the contact block are untouched (SMS registration references them). Trigger: a signing client (Garage Force of Miami) read the old 'monthly subscription / 30 days notice' clauses and asked for written confirmation they did not apply. Last-updated date on the page is now September 4, 2026.
...
- **`pricing.html` and `pricing-internal.html` DELETED** (both were the dead à-la-carte menu — receptionist, reputation, websites; the "internal" one was publicly reachable at /pricing-internal). `_redirects` 301s `/pricing*` and `/pricing-internal*` → `/`. **Do not resurrect a pricing page** — the offer is pay-per-booked-appointment, priced on a call.
- **Homepage refocused on the appointment pipeline.** Hero is now "STOP CHASING LEADS." / CTA "GET BOOKED APPOINTMENTS". The three service tiles that sold other things ("A Reputation That Sells", "Sites That Actually Sell") are gone — replaced by **We Generate The Leads → We Call Them Ourselves → You Get The Appointment**. How-It-Works steps rewritten to the real process (we fund the ads → in-house sales team calls by phone → we book onto the client's calendar → they show up and close). Stats bar, FAQ, contact copy and the animated phone notifications all rewritten to appointments; the phone counter now counts only `booked` events. **The human sales team is the differentiator — never describe the calling as AI/automated.**
- **Contrast fix** on the testimonials section (Jakob: hard to read). Section bg → `--black-shade`, cards → `#24242A` + border (they were `#1F1F21` on `#18181A`, nearly invisible), `.testimonial-role` → `--muted-dark` (was `--muted-mid`, ~2.8:1 — failed WCAG AA; now ~5.5:1), quote text → `--mid-gray`, featured card de-italicised. Colour scheme itself unchanged (gold `#F0A500` + dark).
- Still deliberately untouched: the North Star case study's Lead Lock story, which mentions the AI receptionist as part of Scott's real history (Jakob: "you can leave it under the case studies").
...
- `ai-receptionist.html` — dead offer, now a redirect stub (301 → `/`). Do not revive.
...
- **GHL calendar (`GHL_CALENDAR_BASE`):** `https://api.leadconnectorhq.com/widget/booking/onF95pIwhuvYzJxiPrpx`. GHL's `form_embed.js` auto-resizes the iframe, and it keys off the iframe's `id` — so the `id` in `epoxy.html` must stay `<calendarId>_lp`. Change both together. Calendar's post-booking redirect is set to `/epoxy-thank-you` (fires the Schedule event).
  - **Swapped 2026-09-10** from `vlhhAVmm0RnenlMMEoRm` (Round Robin, Jakob only) to a **Collective** calendar hosting Jakob **and** Jason, so a booking lands on both men's calendars instead of only Jakob's. A GHL calendar's `calendarType` **cannot be changed after creation** — a `PUT` with a new type returns `200 OK` and silently does nothing — so sharing a calendar always means creating a new one and repointing this constant.
